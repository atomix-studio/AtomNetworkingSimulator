    ...
    public class GraphEntityComponent : MonoBehaviour, INodeComponent
    {
        ...
        private void OnJoiningFragmentRequestReceived(FragmentJoiningRequestPacket packet)
        {
            ...
            if (packet.fragmentLevel < _fragmentData.FragmentLevel)
            {
                ...
                // NEW FRAGMENT MASTER SET ?
                for(int i = 0; i < _fragmentData.FragmentMembers.Count; ++i)
                {
                    var entity = WorldSimulationManager.nodeAddresses[_fragmentData.FragmentMembers[i].peerAdress];
                   entity.graphEntityComponent._fragmentData.FragmentOwnerId = _fragmentData.FragmentOwnerId;
                   entity.graphEntityComponent._fragmentData.FragmentLevel = _fragmentData.FragmentLevel;
                   entity.graphEntityComponent._fragmentData.FragmentMembers = _fragmentData.FragmentMembers;
                }
            ...
            for (int i = 0; i < _fragmentData.FragmentMembers.Count; ++i)
            {
                ...
                for (int j = 0; j < crtmember.networkHandling.Connections.Count; ++j)
                {
                    ...
                    if (conmember.graphEntityComponent._fragmentData.FragmentOwnerId != _fragmentData.FragmentOwnerId)
                        outerConnections.Add(new(crtmember.networkHandling.LocalPeerInfo, connpinfo.Value));
                }
            }
...
/*            for (int i = 0; i < WorldSimulationManager.nodeAddresses.Count; ++i)
            {
                ...
                if (crtmember.Value.graphEntityComponent._fragmentData == null)
                    Debug.LogError(1);
...
*/
            ...
            {
                _fragmentData.MinimumOutgoingEdgePeer = (null, null);
